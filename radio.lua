...
RegisterCommand('+radiotalk', function()
    ...
    if not radioPressed and radioEnabled then
        if radioChannel > 0 then
            ...
            if GetConvarInt('voice_enableRadioAnim', 0) == 1 and 
                not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) and 
                not disableRadioAnim then
                ...
                local propName = "prop_cs_hand_radio"
                local boneIndex = GetPedBoneIndex(playerPed, 28422)
                local propPlacement = vector3(0.0750, 0.0230, -0.0230)
                local propRotation = vector3(-90.0, 0.0, -59.9999)

                radioProp = CreateObject(GetHashKey(propName), 0, 0, 0, true, true, true)
                AttachEntityToEntity(radioProp, playerPed, boneIndex, propPlacement.x, propPlacement.y, propPlacement.z, propRotation.x, propRotation.y, propRotation.z, true, true, false, true, 1, true)
